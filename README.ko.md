...
시트로 담을 수 없는 것은 조용히 뭉개지 않고 이름을 댑니다. 기기별 변형, `%#@…@` 치환,
그리고 폴더에 있는 다른 `.strings` 테이블이 그렇습니다. `--table Errors` 로 그중 하나를
대신 읽습니다. 한 번에 하나씩만 읽는 이유는, 서로 다른 테이블이 같은 키를 가질 수 있고
합치면 generate 가 중복 키로 거부하는 시트가 되기 때문입니다.
